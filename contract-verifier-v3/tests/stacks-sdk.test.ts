...
import { ContractverifierSDK } from '../src/stacks-sdk';

describe('ContractverifierSDK', () => {
  let sdk: ContractverifierSDK;

  beforeEach(() => {
    sdk = new ContractverifierSDK(false); // testnet
  });
  ...
  it('should get contract information', async () => {
    ...
    expect(info.name).toBe('contract-verifier-v3');
    expect(info.network).toBe('testnet');
  });

  it('should handle mainnet configuration', () => {
    const mainnetSdk = new ContractverifierSDK(true);
