...
const appMetadata = {
  name: "Crowdfund Platform v2",
  description: "A dApp for interacting with the Crowdfund Platform v2 contract.",
...
const contractName = "crowdfund-platform-v2";
...
function App() {
  ...
  const [title, setTitle] = useState("My Awesome Campaign V2");
  const [goal, setGoal] = useState(1000000); // 1 STX in micro-STX
  const [duration, setDuration] = useState(100); // Blocks
  ...
  const createCampaign = async () => {
    ...
    if (!title || goal <= 0 || duration <= 0) {
        alert("Please enter a valid title, goal, and duration.");
        return;
    }
  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        <div className="card">
          <p>Create a new crowdfunding campaign on Stacks Mainnet (v2).</p>
          <div>
            <label>Title: </label>
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} />
          </div>
          <div>
            <label>Goal (micro-STX): </label>
            <input type="number" value={goal} onChange={(e) => setGoal(parseInt(e.target.value) || 0)} />
          </div>
          <div>
            <label>Duration (blocks): </label>
            <input type="number" value={duration} onChange={(e) => setDuration(parseInt(e.target.value) || 0)} />
          </div>
          <button onClick={createCampaign} disabled={!address || loading || !title || goal <= 0 || duration <= 0}>
