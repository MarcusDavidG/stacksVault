...
import { CrowdfundplatformSDK } from '../src/stacks-sdk';

describe('CrowdfundplatformSDK', () => {
  let sdk: CrowdfundplatformSDK;

  beforeEach(() => {
    sdk = new CrowdfundplatformSDK(false); // testnet
  });
  ...
  it('should get contract information', async () => {
    ...
    expect(info.name).toBe('crowdfund-platform-v3');
    expect(info.network).toBe('testnet');
  });

  it('should handle mainnet configuration', () => {
    const mainnetSdk = new CrowdfundplatformSDK(true);
