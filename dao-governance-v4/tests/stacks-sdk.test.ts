...
import { DaogovernanceSDK } from '../src/stacks-sdk';

describe('DaogovernanceSDK', () => {
  let sdk: DaogovernanceSDK;

  beforeEach(() => {
    sdk = new DaogovernanceSDK(false); // testnet
  });
  ...
  it('should get contract information', async () => {
    ...
    expect(info.name).toBe('dao-governance-v4');
    expect(info.network).toBe('testnet');
  });

  it('should handle mainnet configuration', () => {
    const mainnetSdk = new DaogovernanceSDK(true);
