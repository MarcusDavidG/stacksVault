...
import { DecentralizedlotterySDK } from '../src/stacks-sdk';

describe('DecentralizedlotterySDK', () => {
  let sdk: DecentralizedlotterySDK;

  beforeEach(() => {
    sdk = new DecentralizedlotterySDK(false); // testnet
  });
  ...
  it('should get contract information', async () => {
    ...
    expect(info.name).toBe('decentralized-lottery-v3');
    expect(info.network).toBe('testnet');
  });

  it('should handle mainnet configuration', () => {
    const mainnetSdk = new DecentralizedlotterySDK(true);
