...
const appMetadata = {
  name: "Digital Will V2",
  description: "A dApp for creating digital wills on the Stacks blockchain.",
...
const contractName = "digital-will-v2";
...
function App() {
  ...
  // Will creation state
  const [beneficiaryAddress, setBeneficiaryAddress] = useState("ST1PQHQKV0RJXZFY1DGX8MNSNYVE3FY9H5NGR8D4F"); // Example principal
  const [willAmount, setWillAmount] = useState(1000000); // 1 STX in micro-STX
  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        <div className="card">
          <p>Create a new digital will on Stacks Mainnet.</p>
          <div>
          ...
          <div>
            ...
            <input type="number" value={willAmount} onChange={(e) => setWillAmount(parseInt(e.target.value) || 0)} />
          </div>
          <button onClick={createWill} disabled={!address || loading || willAmount <= 0 || !beneficiaryAddress}>
            {loading ? "Processing..." : "Create Will"}
          </button>
          {txStatus && (
