...
import { InsurancepoolSDK } from '../src/stacks-sdk';

describe('InsurancepoolSDK', () => {
  let sdk: InsurancepoolSDK;

  beforeEach(() => {
    sdk = new InsurancepoolSDK(false); // testnet
  });
  ...
  it('should get contract information', async () => {
    ...
    expect(info.name).toBe('insurance-pool-v4');
    expect(info.network).toBe('testnet');
  });

  it('should handle mainnet configuration', () => {
    const mainnetSdk = new InsurancepoolSDK(true);
