...
describe('LendingpoolSDK', () => {
  let sdk: LendingpoolSDK;

  beforeEach(() => {
    sdk = new LendingpoolSDK(false); // testnet
  });
  ...
  it('should get contract information', async () => {
    ...
    expect(info.name).toBe('lending-pool-v4');
    expect(info.network).toBe('testnet');
  });

  it('should handle mainnet configuration', () => {
    const mainnetSdk = new LendingpoolSDK(true);
