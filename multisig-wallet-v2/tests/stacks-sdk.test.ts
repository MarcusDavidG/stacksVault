...
import { MultisigwalletSDK } from '../src/stacks-sdk';

describe('MultisigwalletSDK', () => {
  let sdk: MultisigwalletSDK;

  beforeEach(() => {
    sdk = new MultisigwalletSDK(false); // testnet
  });
  ...
  it('should get contract information', async () => {
    ...
    expect(info.name).toBe('multisig-wallet-v2');
    expect(info.network).toBe('testnet');
  });

  it('should handle mainnet configuration', () => {
    const mainnetSdk = new MultisigwalletSDK(true);
